...
#Create a figure object
		plt.figure()
		colors = plt.cm.GnBu(np.linspace(0, 1, 24)) #see https://matplotlib.org/tutorials/colors/colormaps.html
		if LEGEND == 'n':
...
#Save the figure to a png
		...
		INPUTFRAME = INPUTFRAME.transpose()
		plt.figure()
		colors = plt.cm.GnBu(np.linspace(0, 1, 24))
		if LEGEND == 'n':
			FIG = INPUTFRAME.plot.bar(stacked=True, legend=False, color=colors)
			FIG.figure.savefig(PREFIX + '_nolegend.png')
		else:
			...
			FIG.figure.savefig(PREFIX + 'GnBu' + '.png', bbox_extra_artists=(lgd,), bbox_inches='tight')
