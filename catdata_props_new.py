...
#Opens genomesizes.txt as a dictionary. Values taken from RepeatMasker output files 
	...
	GENOMESIZES = pd.read_table(SIZEFILE, sep='\t', names=['taxon', 'genomesize'], squeeze=True, index_col=0)
	GENOMESIZESFRAME = pd.read_table(SIZEFILE, sep='\t', names=['taxon', 'genomesize'], squeeze=True)

#Get taxon list from genome sizes file.
	TAXA = GENOMESIZESFRAME['taxon'].tolist()
#TAXA=['dIul', 'aVan', 'eTal', 'hTel', 'hEcal', 'hHim', 'hEra', 'hDem', 'hSar', 'lDor', 'hBur', 'hMel', 'hCyd', 'hTim', 'hNum', 'hBes', 'hEca', 'hEle', 'hPar']
...
#Initialize lists of names
	...
	for TAXON in TAXA:
		print('Reading in RM2bed output for ' + TAXON + '. Writing out as processed.bed file.')
		TAXONRMBED = pd.read_table(TAXON + '_' + PREFIX + '_rm.bed', sep='\t', index_col=False, names=[TAXON + '_chromosome', 'start', 'stop', 'TE', 'size', 'orientation','class', 'family', 'div'])
		TAXONPROCESSEDBED = TAXONRMBED[['TE', 'size', 'class', 'family', 'div']]
		TAXONPROCESSEDBED.columns = [TAXON + '_TE', TAXON + '_size', TAXON + '_class', TAXON + '_family', TAXON + '_div']
#       TAXONPROCESSEDBED.rename(index=str, columns={'TE': TAXON + '_TE', 'size' : TAXON + $
...
#Generate lists of class names, family names, and TE names as well as files that correspond to the classes and families. Too many to save files for each individual TE.
	...
	for TAXON in TAXA:
		...
		TAXONFRAME[TAXON + '_prop'] = TAXONFRAME[TAXON + '_size']/GENOMESIZES[TAXON]	
		#rearrange the columns
		TAXONFRAME = TAXONFRAME[[TAXON + '_TE', TAXON + '_size', TAXON + '_prop', TAXON + '_class', TAXON + '_family', TAXON + '_div']]
...
#For every class name split the dataframe into a separate dataframe and save it
	...
	for TAXON in TAXA:	
		DNANAME = pd.read_table('class_files/' + TAXON + '_DNA_' + PREFIX + '_class_processed_beds.txt', sep='\t', index_col=False, names=[TAXON + '_TE', TAXON + '_size', TAXON + '_class', TAXON + '_family', TAXON + '_div'])
		SINEFRAME = pd.read_table('class_files/' + TAXON + '_SINE_' + PREFIX + '_class_processed_beds.txt', sep='\t', index_col=False, names=[TAXON + '_TE', TAXON + '_size', TAXON + '_class', TAXON + '_family', TAXON + '_div'])
		LINEFRAME = pd.read_table('class_files/' + TAXON + '_LINE_' + PREFIX + '_class_processed_beds.txt', sep='\t', index_col=False, names=[TAXON + '_TE', TAXON + '_size', TAXON + '_class', TAXON + '_family', TAXON + '_div'])
		RCFRAME = pd.read_table('class_files/' + TAXON + '_RC_' + PREFIX + '_class_processed_beds.txt', sep='\t', index_col=False, names=[TAXON + '_TE', TAXON + '_size', TAXON + '_class', TAXON + '_family', TAXON + '_div'])
		LTRFRAME = pd.read_table('class_files/' + TAXON + '_LTR_' + PREFIX + '_class_processed_beds.txt', sep='\t', index_col=False, names=[TAXON + '_TE', TAXON + '_size', TAXON + '_class', TAXON + '_family', TAXON + '_div'])
...
def get_args():
	parser = argparse.ArgumentParser(description="Will process an rm.bed file output from RM2bed.py into various subfiles for downstream processing.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
